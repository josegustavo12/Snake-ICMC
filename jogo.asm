...
;	O jogo funciona da seguinte maneira, o primeiro a jogar e o usuario, que pode comprar at� 5 cartas no intuito de somar 21, feito
;isso e a vez da casa que segue o algoritmo de comprar cartas at� somar no m�nimo 17 pontos, quando ela tiver em m�os esse tanto de cartas
;ela para de comprar cartas(sempre obedecendo o limite de 5 cartas). As cartas da casa entram todas, com excess�o da primeira, viradas
;assim possibilitando ao jogador antes da mesma revelar suas cartas compradas dobrar a aposta atual do jogo.
;	Por fim a casa revela suas cartas e s�o comparados os valores somados pelos dois participantes, caso nenhum dos dois passe de 21
...
;se o usu�rio somar 22 e a casa 23 quem ganha � o usu�rio.
;	O c�digo segue a seguinte estrutura, primeiro temos as declara��es das strings e "telas" que ser�o usadas no jogo, das variaveis e
;tambem de um vetor de 1040 n�meros aleat�rios que simboliza as cartas indo de 1 a 10 sendo 10 representado pela figura K. Depois disso
;temos a fun��o main, onde as variaveis sao inciadas e o loop do jogo acontece. Importante ressaltar que o acesso ao vetor de numeros
;aleat�rios acontece atravez da tecla digitada no come�o do jogo, que serve de semente para o acesso.
;	Depois da fun��o main seguem todas as outras fun��es utilizadas, sendo as primeiras de realiza��o da l�gica do jogo e as ultimas
;relacionadas � impress�o das cartas.
; 	Para a implementa��o desse jogo o mapa de caracteres foi alterado.
; ------- TABELA DE CORES -------
...
money : var #1			;	voce come�a com 10 dinheiros
...
main:
	...
	;na main os registradores r6,r7 e r3 nao devem ser modificados fora de sua fun��o original;
	loadn r0, #0
	loadn r1, #0
	loadn r2, #0
	loadn r3, #0
	...
	loadn r6, #0
	loadn r7, #0
	store numero_da_rodada, r0					;come�a sendo a rodada 0
...
primeira_rodada:
	...
	loadn r0, #1
	store rodadaflag, r0
	;----- condi��es para o jogo acabar :	1 - acabar seus dinheiros, 2 - a casa falir, vc chegar a 100 dinheiros -----;
	loadn r1, #0
	load r2, money
	cmp r2, r1
	jel ZERO_MONEY
...
IMPRIME_SEU_DINHEIRO_E_APOSTA_DA_RODADA:
	...
	call Imprimestr
	load r0, money
	loadn r1, #10		;comparar r0 com dez pra ver se sera printado com qual fun��o
	cmp r0, r1
...
ENDROUND:
	...
	loadn r6, #500
	call PrintR5R6
	;----------- aqui serao comparados os dois valores de m�os de usuario e casa --------------;
	call Delay
	load r0, somauser
	load r1, somamaq
...
COMPARACAO_FINAL:
	loadn r7, #1		;flag de compara��o, ver se os dois estouraram ou nao
	cmp r3, r7
	jeq BOTH_ESTOURARAM
	jmp NAO_ESTOURARAM
BOTH_ESTOURARAM:
...
CHECKAPOSTA:
	...
	jeq CHECKAPOSTAOUT	; se o usuario digitou n saio da fun�ao
...
SETRANDUSER:
	;compensa os rands rodados dentro das fun��es nao computados no r6
	push r0
	push r1
...
PRINTAFIGURA:
	...
	loadn r3, #'K'
	loadn r2, #2304
	add r3, r2, r3 ;add cor no caractere do 
	outchar r3, r4
	pop r7
	pop r6
	pop r5
...
BUYMAISQUESTION:
	...
	call ATUALIZARAND
	call CHAMAIMPRESSAO	;fun��o de switch case
...
STARTRAND:
	;aqui nessa fun��o nao tem push ou pop pois r6 e 7 ficam forever usados na fun�ao main;
...
ATUALIZARAND:
	...
	loadn r2, #1040
	loadn r0, #RAND
	add r2,r2,r0 			;somo o come�o do vetor rand com 1040
...
ATUALIZARAND_EXCESSAO:
	loadn r6, #RAND			;caso isso aconte�a, irei para o come�o do vetor
ATUALIZARAND_FIM:
	pop r2
	pop r0
	rts
...
;--------------------------------------- AQUI ESTAO AS FUN��ES DE IMPRESS�O DAS CARTAS --------------------------------------------;
;---------------------------------------FUN��ES DE IMPRESS�O DAS CARTAS VIRADAS DA CASA -------------------------------------------;
IMPRIMIR2CARTAS:
	push r0	
	push r1	
	push r2	
...
;------------------------------------- fun��es de impressao com o numero da casa ---------------------------------------------------;
